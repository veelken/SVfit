...
namespace 
{
  ...
  class AuxResonancePtEtaPhiMassValue : public ROOT::Math::Functor
  {
   public:
    ...
    enum { kPt, kEta, kPhi, kMass, kVisPt, kVisEta, kVisPhi, kVisMass };
   private:
    virtual double DoEval(const double* x) const
    {
      ...
      else if ( variable_ == kVisPt   ) retVal = resonance->p4().pt();
      else if ( variable_ == kVisEta  ) retVal = resonance->p4().eta();
      else if ( variable_ == kVisPhi  ) retVal = resonance->p4().phi();
      else if ( variable_ == kVisMass ) retVal = resonance->p4().mass();
      else assert(0);
      return retVal;
    } 
  ...
  class AuxDaughterPtEtaPhiMassValue : public ROOT::Math::Functor
  {
   ...
   private:
    virtual double DoEval(const double* x) const
    {
      ...
      else if ( variable_ == kVisPt   ) retVal = daughter->p4().pt();
      else if ( variable_ == kVisEta  ) retVal = daughter->p4().eta();
      else if ( variable_ == kVisPhi  ) retVal = daughter->p4().phi();
      else if ( variable_ == kVisMass ) retVal = daughter->p4().mass();
      else assert(0);
      return retVal;
    } 
    std::string branchName_;
...
void SVfitAlgorithmByIntegration2::beginJob()
{
  ...
  if ( monitorMarkovChain_ ) {
    ...
    for ( std::vector<resonanceModelType*>::const_iterator resonance = eventModel_->resonances_.begin();
	  resonance != eventModel_->resonances_.end(); ++resonance ) {
      const std::string& resonanceName = (*resonance)->resonanceName_;
      for ( int variable = AuxResonancePtEtaPhiMassValue::kPt; variable <= AuxResonancePtEtaPhiMassValue::kVisMass; ++variable ) {
	...
	else if ( variable == AuxResonancePtEtaPhiMassValue::kVisPt   ) branchName.append("VisPt");
	else if ( variable == AuxResonancePtEtaPhiMassValue::kVisEta  ) branchName.append("VisEta");
	else if ( variable == AuxResonancePtEtaPhiMassValue::kVisPhi  ) branchName.append("VisPhi");
	else if ( variable == AuxResonancePtEtaPhiMassValue::kVisMass ) branchName.append("VisMass");
	else assert(0);
	...
	integrator_->setF(*auxResonanceValue, branchName);
      ...
      for ( std::vector<daughterModelType*>::const_iterator daughter = (*resonance)->daughters_.begin();
	    daughter != (*resonance)->daughters_.end(); ++daughter ) {
	const std::string& daughterName = (*daughter)->daughterName_;
	for ( int variable = AuxDaughterPtEtaPhiMassValue::kPt; variable <= AuxDaughterPtEtaPhiMassValue::kVisMass; ++variable ) {
	  ...
	  else if ( variable == AuxDaughterPtEtaPhiMassValue::kVisPt   ) branchName.append("VisPt");
	  else if ( variable == AuxDaughterPtEtaPhiMassValue::kVisEta  ) branchName.append("VisEta");
	  else if ( variable == AuxDaughterPtEtaPhiMassValue::kVisPhi  ) branchName.append("VisPhi");
	  else if ( variable == AuxDaughterPtEtaPhiMassValue::kVisMass ) branchName.append("VisMass");
	  else assert(0);
...
//--- set central values and uncertainties on reconstructed masses
  if ( errorFlag == 0 ) {
	...
	double ptMaximum, ptMaximum_interpol, ptMean, ptQuantile016, ptQuantile050, ptQuantile084, ptMean3sigmaWithinMax, ptMean5sigmaWithinMax;
        extractHistogramProperties(
          histogramPt_resonance, histogramPt_resonance_density,
          ptMaximum, ptMaximum_interpol, ptMean, ptQuantile016, ptQuantile050, ptQuantile084, ptMean3sigmaWithinMax, ptMean5sigmaWithinMax,
	  0);
	double pt = ptMaximum_interpol;
	double etaMaximum, etaMaximum_interpol, etaMean, etaQuantile016, etaQuantile050, etaQuantile084, etaMean3sigmaWithinMax, etaMean5sigmaWithinMax;
        extractHistogramProperties(
          histogramEta_resonance, histogramEta_resonance_density,
          etaMaximum, etaMaximum_interpol, etaMean, etaQuantile016, etaQuantile050, etaQuantile084, etaMean3sigmaWithinMax, etaMean5sigmaWithinMax,
	  0);
	double eta = etaMaximum_interpol;
	double phiMaximum, phiMaximum_interpol, phiMean, phiQuantile016, phiQuantile050, phiQuantile084, phiMean3sigmaWithinMax, phiMean5sigmaWithinMax;
        extractHistogramProperties(
          histogramPhi_resonance, histogramPhi_resonance_density,
          phiMaximum, phiMaximum_interpol, phiMean, phiQuantile016, phiQuantile050, phiQuantile084, phiMean3sigmaWithinMax, phiMean5sigmaWithinMax,
	  0);
  ...
  if ( errorFlag == 0 ) {
	...
	0);
      ...
      else if ( max_or_median_ == kMean3sigmaWithinMax ) fitParameterValues_[iDimension] = valueMean3sigmaWithinMax;
      else if ( max_or_median_ == kMean5sigmaWithinMax ) fitParameterValues_[iDimension] = valueMean5sigmaWithinMax;
...
void SVfitAlgorithmByIntegration2::setMassResults(SVfitResonanceHypothesisBase* resonance, const TH1* histMassResult) const
{
  ...
  if ( histMassResult_density->Integral() > 0. ) {
    double massMaximum, massMaximum_interpol, massMean, massQuantile016, massQuantile050, massQuantile084, massMean3sigmaWithinMax, massMean5sigmaWithinMax;
    extractHistogramProperties(
      histMassResult, histMassResult_density,
      massMaximum, massMaximum_interpol, massMean, massQuantile016, massQuantile050, massQuantile084, massMean3sigmaWithinMax, massMean5sigmaWithinMax,
      verbosity_);
    
    double mass;
    if      ( max_or_median_ == kMax                 ) mass = massMaximum_interpol;
    else if ( max_or_median_ == kMedian              ) mass = massQuantile050;
    else if ( max_or_median_ == kMean3sigmaWithinMax ) mass = massMean3sigmaWithinMax;
    else if ( max_or_median_ == kMean5sigmaWithinMax ) mass = massMean5sigmaWithinMax;
    else assert(0);
