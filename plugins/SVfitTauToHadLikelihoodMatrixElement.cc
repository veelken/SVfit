#include "TauAnalysis/SVfit/plugins/SVfitTauToHadLikelihoodMatrixElement.h"
...
SVfitTauToHadLikelihoodMatrixElement::SVfitTauToHadLikelihoodMatrixElement(const edm::ParameterSet& cfg)
    ...
    algorithm_(0)
{
  ...
  if ( !inputFileName.isLocal() ) 
    throw cms::Exception("SVfitTauToHadLikelihoodMatrixElement") 
...
SVfitTauToHadLikelihoodMatrixElement::~SVfitTauToHadLikelihoodMatrixElement()
{
  delete inputFile_;
...
void SVfitTauToHadLikelihoodMatrixElement::beginJob(SVfitAlgorithmBase* algorithm)
{
...
void SVfitTauToHadLikelihoodMatrixElement::beginCandidate(const SVfitSingleParticleHypothesis* hypothesis)
{
  ...
  if ( this->verbosity_ ) std::cout << "<SVfitTauToHadLikelihoodMatrixElement::beginCandidate>:" << std::endl;
...
double SVfitTauToHadLikelihoodMatrixElement::compProb_pionDecay(double visEnFracX, int polSign, double tauLeptonPt) const
{
...
double SVfitTauToHadLikelihoodMatrixElement::compProb_rhoDecay(double visEnFracX, double z, int polSign, double tauLeptonPt) const
{
//--- compute probability for observed tau decay products 
//    to be compatible with tau- --> rho- nu --> pi- pi0 nu decay
  ...
  if ( mL <= 0. && mT <= 0. ) {
    if ( numWarningsUnphysicalDecay_rho_ < 3 ) 
      edm::LogWarning ("SVfitTauToHadLikelihoodMatrixElement::compProb_rhoDecay")
...
double SVfitTauToHadLikelihoodMatrixElement::compProb_a1Decay(double visEnFracX, double z, int polSign, double tauLeptonPt) const
{
//--- compute probability for observed tau decay products 
//    to be compatible with tau- --> a1- nu --> pi- pi0 p0 nu and tau- --> a1- nu --> pi+ pi- pi- nu decays
  ...
  if ( mL <= 0. && mT <= 0. ) {
    if ( numWarningsUnphysicalDecay_a1_ < 3 ) 
      edm::LogWarning ("SVfitTauToHadLikelihoodMatrixElement::compProb_a1Decay")
	<< "Failed to compute vector meson line-shape integral, xCut = " << xCut << " !!" << std::endl;
    mL = 1.e-3;
    mT = 1.e-3;
...
double SVfitTauToHadLikelihoodMatrixElement::operator()(const SVfitSingleParticleHypothesis* hypothesis, int polSign) const
{
...
//
  ...
  //if ( this->verbosity_ ) std::cout << "<SVfitTauToHadLikelihoodMatrixElement::operator()>:" << std::endl;
...
DEFINE_EDM_PLUGIN(SVfitSingleParticleLikelihoodPluginFactory, SVfitTauToHadLikelihoodMatrixElement, "SVfitTauToHadLikelihoodMatrixElement");
