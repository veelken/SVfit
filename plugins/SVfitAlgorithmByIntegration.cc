...
void SVfitAlgorithmByIntegration::fitImp() const
{
  ...
  if ( numMassParameters_ == 1 ) {
    histResults = new TH1D(histResultsName.str().data(), histResultsName.str().data(), 
    ...
    histResults = new TH2D(histResultsName.str().data(), histResultsName.str().data(), 
			   fitParameterReplacements_[0]->numGridPoints_, fitParameterReplacements_[0]->resBinning_->GetArray(),
...
//    to perform actual integration
    ...
    if      ( numMassParameters_ == 1 ) {
      int bin = histResults->FindBin(massParameterValues[0]);
      histResults->SetBinContent(bin, p);
      histResults->SetBinError(bin, pErr);
    } else if ( numMassParameters_ == 2 ) {
      TH2* histResults2d = dynamic_cast<TH2*>(histResults);
      assert(histResults2d);
      int bin = histResults2d->FindBin(massParameterValues[0], massParameterValues[1]);
      histResults2d->SetBinContent(bin, p);
      histResults2d->SetBinError(bin, pErr);
    } else assert(0);
       ...
       SVfitResonanceHypothesisByIntegration* resonance, const TH1* histMassResults, unsigned iDimension) const
{
  ...
  if ( histMassResult1d_density->Integral() > 0. ) {
    ...
    resonance->massMean3sigmaWithinMax_ = massMean3sigmaWithinMax;
    resonance->massMean5sigmaWithinMax_ = massMean5sigmaWithinMax;
