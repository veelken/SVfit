    ...
    vertexFitAlgorithm_(0)
{
  ...
  else throw cms::Exception("SVfitEventVertexRefitter")
...
//--- get pointer to TransientTrackBuilder
  ...
  if ( !trackBuilder_ ) 
    throw cms::Exception("SVfitEventVertexRefitter::beginEvent")
...
TransientVertex SVfitEventVertexRefitter::refit(const reco::Vertex* eventVertex, const std::vector<const reco::Track*>* svTracks) const
{
//--- return (invalid) dummy vertex in case primary event vertex cannot be refitted,
//    due to insufficient information

  if ( verbosity_ >= 1 ) {
    std::cout << "<SVfitEventVertexRefitter::refit>:" << std::endl;
...
//    refit the primary event vertex with all tracks used in the original primary event vertex fit
  TransientVertex eventVertex_refitted;
  if ( pvTracks_refit.size() >= minNumTracksRefit_ ) {
    if ( applyBeamSpotConstraint_ && beamSpotIsValid_ ) eventVertex_refitted = vertexFitAlgorithm_->vertex(pvTracks_refit, *beamSpot_);
    else eventVertex_refitted = vertexFitAlgorithm_->vertex(pvTracks_refit);
  } else {
    ...
    if ( applyBeamSpotConstraint_ && beamSpotIsValid_ ) eventVertex_refitted = vertexFitAlgorithm_->vertex(pvTracks_original, *beamSpot_);
    else eventVertex_refitted = vertexFitAlgorithm_->vertex(pvTracks_original);
  }
  if ( verbosity_ >= 1 ) {
    AlgebraicSymMatrix33 eventVertexCov = eventVertex_refitted.positionError().matrix_new();
    std::cout << " refitted vertex: x = " << eventVertex_refitted.position().x() << " +/- " << TMath::Sqrt(eventVertexCov(0, 0)) << ","
	      << " y = " << eventVertex_refitted.position().y() << " +/- " << TMath::Sqrt(eventVertexCov(1, 1)) << ","
	      << " z = " << eventVertex_refitted.position().z() << " +/- " << TMath::Sqrt(eventVertexCov(2, 2)) << std::endl;
  }

  return eventVertex_refitted;
}
