...
#ifdef SVFIT_DEBUG 
    if ( verbosity_ >= 2 ) std::cout << " deltaE = " << deltaE << std::endl;
#endif
    pAccept = TMath::Exp(-deltaE);
  } else if ( moveMode_ == kHybrid     ) { // Hybrid algorithm: accept moves according to eqs. (19, (25) and (26) in [2]
    double Ks = evalK(p_, 0, numDimensions_);
    double KsProposal = evalK(pProposal_, 0, numDimensions_);
    double deltaH = deltaE + (KsProposal - Ks);
#ifdef SVFIT_DEBUG 
    if ( verbosity_ >= 2 ) std::cout << " deltaH = " << deltaH << std::endl;
#endif 
    double Kd = evalK(p_, numDimensions_, 2*numDimensions_);
    double base = 1. - deltaH/Kd;
    double rho = ( base > 0. ) ? 
      TMath::Power(base, 0.5*numDimensions_ - 1.) : 0.;
    pAccept = rho;
  } else assert(0);
#ifdef SVFIT_DEBUG 
  if ( verbosity_ >= 2 ) std::cout << "p(accept) = " << pAccept << std::endl;
#endif
...
void MarkovChainIntegrator::openMonitorFile(const std::string& monitorFileName)
{
  ...
  branchNameIdx_  = "move";
  branchValueIdx_ = 0;
  monitorTree_->Branch(branchNameIdx_.data(), &branchValueIdx_, Form("%s/I", branchNameIdx_.data()));
